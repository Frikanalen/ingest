...
Capacity is `replicas`, and nothing else — a worker asks for a job only when it is free, so no dispatcher has to know how many there are. **It must not be zero**: member uploads are drained by this pool, and a job nobody claims sits at `pending` indefinitely with nothing to notice or complain. It ships at three, one for each node in the cluster. A replica count is not a placement, so a soft `topologySpreadConstraint` over `kubernetes.io/hostname` is what actually keeps them apart — soft rather than `DoNotSchedule`, because a constraint a replacement cannot satisfy is a Pending pod, which is how this pool deadlocked once already. Scale it up for a backfill:
