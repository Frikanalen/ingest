...
Capacity is `replicas`, and nothing else — a worker asks for a job only when it is free, so no dispatcher has to know how many there are. **It must not be zero**: member uploads are drained by this pool, and a job nobody claims sits at `pending` indefinitely with nothing to notice or complain. It ships at four, one for each node in the cluster — a number to keep in step with the node count, since the hard spread constraint below means a replica with no node of its own stays Pending rather than doubling up. A replica count is not a placement, so a hard `topologySpreadConstraint` over `kubernetes.io/hostname` keeps them apart: `maxSkew: 1` with `DoNotSchedule` leaves a worker Pending rather than placing two workers on one node while another eligible node has none. A cordoned, full, or otherwise ineligible node can therefore block a rollout until capacity is restored. Scale it up for a backfill:
...
Rollouts therefore retire workers before starting their replacements (`maxSurge: 0`, `maxUnavailable: 2`) rather than the other way around. Two at a time means a deploy that catches every worker mid-encode is two drains long rather than three; the cost is that one worker is guaranteed during a rollout rather than three, which suits a queue that waits patiently. At two replicas the default rounds to the opposite, which deadlocks: nothing may be terminated until the replacement is Ready, the replacement cannot schedule until the outgoing worker's CPU request is freed, and the outgoing worker is never signalled, so it never frees it. `progressDeadlineSeconds` is set above the grace period for the same reason — otherwise a rollout that correctly waits out an encode is reported as a failed one.
