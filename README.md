...
Capacity is `replicas`, and nothing else — a worker asks for a job only when it is free, so no dispatcher has to know how many there are. **It must not be zero**: member uploads are drained by this pool, and a job nobody claims sits at `pending` indefinitely with nothing to notice or complain. It ships at three, one for each node in the cluster. A replica count is not a placement, so a hard `topologySpreadConstraint` over `kubernetes.io/hostname` keeps them apart: `maxSkew: 1` with `DoNotSchedule` leaves a worker Pending rather than placing two workers on one node while another eligible node has none. A cordoned, full, or otherwise ineligible node can therefore block a rollout until capacity is restored. Scale it up for a backfill:
...
Worker rollouts use `Recreate`: all old workers are told to retire, and Kubernetes waits for them to finish terminating before it creates any replacements. `RollingUpdate` with `maxSurge: 0` is not sufficient because it creates a replacement as soon as an old worker becomes unavailable, while the terminating pod can retain its CPU reservation for the entire grace period. During a simultaneous, independent upload rollout, those replacement workers can take the capacity released by the old upload pod and leave its replacement Pending — returning 503 until every previous-version worker has gone. With `Recreate`, the queue temporarily has no processing capacity, but jobs wait safely and the upload pod can restart as soon as its own rollout permits. A rollout that catches all three workers mid-encode takes one drain period rather than two. `progressDeadlineSeconds` remains above the grace period so a rollout that correctly waits out an encode is not reported as failed.
